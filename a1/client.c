...
int wellformatted(char *buffer) {
	...
	if(count == 1 && buffer[strlen(buffer)-2] == ';') return 0;
	if(count == 1 && buffer[0] == ';') return 0;
	if(count>1) return 0;
	if(count<1) return 0;
	return 1;
}
...
int main() {
	...
	readinput(buffer); //Process user input
	...
	//write(newsockfd, "Got your message\n", 19);
}